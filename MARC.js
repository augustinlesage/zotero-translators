...
function cleanTitle(value) {
	...
	// valueOne = value.replace(/([^:]+) : .+/, `$1`);
	// valueTwo = value.replace(/[^:]+ : (.+)/, `$1`);
	// valueTwo = valueTwo[0].toUpperCase() + valueTwo.slice(1);
	...
	if (value.match(/(?![Vv]ie)\b(([Xx]{0,2})([Xx]|[Vv]|[Ii][Xx]|[Ii][Vv]|[Vv]?[Ii]{1,3}))[eEᵉ]\b/)) {
		value = value.replace(/(?![Vv]ie)\b(([Xx]{0,2})([Xx]|[Vv]|[Ii][Xx]|[Ii][Vv]|[Vv]?[Ii]{1,3}))[eEᵉ]\b/g, function ($0, $1) { return $1.toUpperCase() + 'ᵉ' });
	...
	// if (value.match(/(?![Vv]ie)\b(([Xx]{0,2})([Xx]|[Vv]|[Ii][Xx]|[Ii][Vv]|[Vv]?[Ii]{1,3}))[eEᵉ]\b/)) {
	// 	value = value.replace(/(?![Vv]ie)\b(([Xx]{0,2})([Xx]|[Vv]|[Ii][Xx]|[Ii][Vv]|[Vv]?[Ii]{1,3}))[eEᵉ]\b/g, function($0,$1) {return '<span style="font-variant:small-caps;">' + $1.toLowerCase() + '</span>ᵉ'});
	// }

	// Lexique
	value = value.replace(/[Mm]oyen[  ][aâ]ge/, 'Moyen Âge');
	value = value.replace(/oeuvre/, 'œuvre');
	value = value.replace(/O[Ee]uvre/, 'Œuvre');
	value = value.replace(/\?uvre/, 'œuvre');
...
record.prototype.translate = function (item) {
	...
	if ((this.getFieldSubfields("200")[0]) && (!(this.getFieldSubfields("245")[0]))) {
		...
		var titleTag = this.getFieldSubfields("200");
		if (titleTag) {
			titleTag = titleTag[0];
			var titleText = titleTag.a;
			if (titleTag.e) {
				if (!/^[,.:;-]/.exec(titleTag.e)) {
					item.shortTitle = titleText;
					titleText += ". ";
				}
				titleText += titleTag.e[0].toUpperCase() + titleTag.e.substring(1);
			}
			if (titleTag.h) {
		...
		// Extract pages and number of volumes if available
		if (!item.pages) {
			var dimTag = this.getFieldSubfields("215");
			for (let j in dimTag) {
				var dim = dimTag[j];
				if (dim.a) {
					var pages = /[^\dMDCLXVImdclxvi]*([MDCLXVImdclxvi]*-*\d+)\s+(p\.?|pages).*/.exec(dim.a);
					if (pages) {
						item.numPages = pages[1].toLowerCase();
					}
		...
		// Extract series
		this._associateDBField(item, "225", "ai", "series");
		// Extract series number
		this._associateDBField(item, "225", "v", "seriesNumber");

		// Repository
		item.libraryCatalog = "BnF Catalogue général (http:// catalogue.bnf.fr)";

		// Extract call number
		item.callNumber = "";
		var coteTag = this.getFieldSubfields("930");
		...
		// Country (102a)
		this._associateDBField(item, "102", "a", "country");
		...
		// function getURLattachment(record, newItem) {
		// 	// Store perennial url from 003 as attachment and accession number
